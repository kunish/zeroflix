...
enum OpenSubtitlesError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConfigured: return "未配置 OpenSubtitles API Key（在设置中填写）"
        case .badStatus(let code, let msg): return msg ?? "OpenSubtitles 请求失败（\(code)）"
        case .noLink: return "未获取到字幕下载链接"
        case .decodeFailed: return "字幕解析失败"
        case .quotaExceeded(let msg): return msg ?? "今日字幕下载配额已用尽"
        }
    }
}
...
actor OpenSubtitlesService {
    ...
    private let userAgent = "Reflix v\(AppConfig.plexVersion)"
    ...
    nonisolated static func languageLabel(_ code: String) -> String {
        ...
        case "zh-cn", "zh", "zh-hans": return "简体中文"
        case "zh-tw", "zh-hk", "zh-hant": return "繁體中文"
        case "en": return "英语"
        case "ja": return "日语"
        case "ko": return "韩语"
        case "fr": return "法语"
